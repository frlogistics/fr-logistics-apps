...
//                ShipStation /orders accepts customField1 as a filter but
//                applies it loosely (returns ~48 instead of 14 for MXS),
//                so we re-filter client-side using exact case-insensitive
//                match on advancedOptions.customField1 of each order.
//   - ss_store → filter /shipments directly by storeId.
//   - portal   → 'unsupported' mode (no ShipStation integration yet).
//
// Why orderId (not orderNumber): orderNumber is a marketplace string and
// can collide between clients ("1047", "1048", etc.). orderId is
// ShipStation's internal primary key — guaranteed unique.
//
// Verified for MXS Overseas Ltd: /orders returns 48, defense-in-depth
// narrows to 14, /shipments returns 43, intersection by orderId = 14
// with tracking. Matches the 14 checked rows in ShipStation UI.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Cache check. We do NOT cache when the response is empty/error so
    // partial deploys don't poison the cache.
    ...
    // Step 1: get the universe of shipments for the window.
    const allShipments = await fetchShipments(startDate, endDate);

    // Step 2: build a Set of orderIds belonging to this client.
    ...
    if (billingSource === 'ss_cf1') {
      ...
      // /orders accepts customField1 as a filter param but applies it
      // loosely. We must re-filter client-side using exact match on
      // advancedOptions.customField1 of each returned order.
    ...
    // Step 3: filter shipments to this client.
    let matched;
    if (billingSource === 'ss_cf1') {
    ...
    // Step 4: keep only those with a tracking number and not voided.
    const shipped = matched.filter((sh) => sh.trackingNumber && !sh.voided);

    // Step 5: project to UI rows.
