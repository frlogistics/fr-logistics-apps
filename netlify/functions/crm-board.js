...
async function buildBoard() {
  ...
  // Age is measured from created_at for every lead, worked or not.
  // updated_at moves on any edit — including bulk data maintenance — so
  // anchoring to it made the leads that had been sitting longest report
  // zero days idle, which is exactly backwards.
  const leads = rawLeads.map(l => {
    const anchor = l.created_at || l.updated_at || '';
    return {
      ...l,
