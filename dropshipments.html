...
// ─── Query builders ──────────────────────────────────────────────────────────
const SELECT_CORE = "id,client_id,tracking_number,order_id,carrier,content,qty_boxes,notes,label_url,label_filename,outbound_carrier,outbound_platform,outbound_tracking,status,group_seq,group_total,email_received_at,physical_received_at,labeled_at,shipped_at,received_by,shipped_by,exception_reason,manifest_id,created_at,updated_at";
...
export default async function handler(req) {
  ...
  try {
    ...
    if (req.method === "GET" && action === "lookup") {
      ...
      // Day 6: limit raised from 5 to 20. A consolidated inbound package can
      // hold several outbound orders under the SAME tracking; truncating the
      // result would hide orders from the operator at the scanner, which is
      // exactly the failure this feature exists to prevent.
      const rows = await sbSelect("dropshipments", `?${filter}&select=${SELECT_WITH_CLIENT}&order=group_seq.asc.nullslast,created_at.asc&limit=20`);
      attachConfigs(rows, configMap);
      ...
      // Group summary for the scanner banner. `declared` is what the client
      // said the box contains ("Ordenes en este paquete: 3"); `registered` is
      // how many rows actually exist. declared > registered means some emails
      // haven't arrived yet — the operator can still receive what's here.
      const inbound = matches.filter(r => r.match_field === "tracking_number");
      const declared = Math.max(0, ...inbound.map(r => r.group_total || 0));
      let group = null;
      if (declared > 1 || inbound.length > 1) {
        group = {
          tracking_number: inbound[0]?.tracking_number || tracking,
          declared: declared || inbound.length,
          registered: inbound.length,
          complete: declared ? inbound.length >= declared : true,
          missing: declared ? Math.max(0, declared - inbound.length) : 0
        };
      }

      return jRes({ rows: matches, count: matches.length, tracking, group });
    }
    ...
    if (req.method === "POST") {
      ...
      const cur = await sbSelect("dropshipments", `?id=eq.${id}&select=id,status,label_url,email_message_id,tracking_number,outbound_tracking,client_id,carrier,order_id,content,outbound_carrier,manifest_id,group_seq,group_total&limit=1`);
      if (!cur.length) return jRes({ error: "not found" }, 404);
      const current = cur[0];
      ...
      // ── Day 6: Ship guard for consolidated packages ───────────────────────
      // One inbound box can carry several outbound orders (Amazon merges
      // purchases). The danger is NOT shipping them one at a time — that's
      // normal, the operator works through them. The danger is shipping the
      // first one and treating the box as finished, leaving the siblings on
      // the bench with nothing on screen to say they exist.
      //
      // So this is a CONFIRMATION, not a rejection. The first attempt returns
      // 409 with the list of siblings still pending; the UI shows them and
      // re-sends with confirm_group:true, which passes. The operator can never
      // ship 1 of 3 without having been shown what's left, and never gets
      // deadlocked either.
      if (act === "ship" && (current.group_total || 0) > 1 && body.confirm_group !== true) {
        const sibs = await sbSelect("dropshipments",
          `?client_id=eq.${current.client_id}&tracking_number=eq.${encodeURIComponent(current.tracking_number)}&id=neq.${id}&select=id,order_id,group_seq,status,content&order=group_seq.asc.nullslast`
        );
        const notShipped = sibs.filter(s => s.status !== "shipped");
        if (notShipped.length) {
          return jRes({
            error: "this box holds other orders that haven't shipped yet",
            code: "GROUP_INCOMPLETE",
            tracking_number: current.tracking_number,
            shipping: { order_id: current.order_id, group_seq: current.group_seq },
            group_total: current.group_total,
            pending: notShipped.map(s => ({
              id: s.id,
              order_id: s.order_id,
            ...
            hint: `Esta caja trae ${current.group_total} órdenes. Al despachar esta quedan ${notShipped.length} sin despachar — no cierres la caja todavía.`
          }, 409);
        }
      }
